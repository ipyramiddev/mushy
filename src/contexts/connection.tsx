...
import { Account, clusterApiUrl, Connection, Commitment, Keypair, Transaction, TransactionInstruction,Blockhash, FeeCalculator,TransactionSignature, SimulatedTransactionResponse,RpcResponseAndContext,SignatureStatus } from "@solana/web3.js";
...
export function ConnectionProvider({ children = undefined as any }) {
  ...
  const openConnection = useCallback(
    () => {
      ...
      setSendConnection(new Connection(endpoint, "singleGossip"));
    },
    [endpoint],
  );
...
export function useOpenConnection() {
  return useContext(ConnectionContext).openConnection;
}
...
export const sendTransaction = async (
  ...
  if (awaitConfirmation) {
    ...
    if (status?.err) {
      ...
      toast.error(<Notification title="Transaction failed..." description={errors}/>);
